...
<script>
    function search_report() {
        ...
        setStateGet('adminTable','<?php echo SECURE_PATH;?>viewreport2/process.php','tableDisplay=1&search=true&from_date='+from_date+'&to_date='+to_date+'&district_sel='+district_sel+'&entry_type='+entry_type);
    }
</script>
...
if(isset($_GET['tableDisplay'])){
    ...
    $s = $database->query("select count(id) as cnt from form2 WHERE status in (1,2,3,4) $con");
    $d1 = mysqli_fetch_array($s);

    $ap = $database->query("select count(id) as cnt from form2 WHERE status in (2,4) $con");
    $d2 = mysqli_fetch_array($ap);

    $p = $database->query("select count(id) as cnt from form2 WHERE status=4 $con");
    $d3 = mysqli_fetch_array($p);

    $r = $database->query("select count(id) as cnt from form2 WHERE status=3 $con");
