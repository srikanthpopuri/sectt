...
<script>
    function search_report() {
        ...
        setStateGet('adminTable','<?php echo SECURE_PATH;?>viewreport4/process.php','tableDisplay=1&search=true&from_date='+from_date+'&to_date='+to_date+'&district_sel='+district_sel+'&entry_type='+entry_type);
    }
</script>
...
if(isset($_GET['tableDisplay'])){
    ...
    $s = $database->query("select count(id) as cnt from form5 WHERE status in (1,2,3,4) $con");
    $d1 = mysqli_fetch_array($s);

    $ap = $database->query("select count(id) as cnt from form5 WHERE status in (2,4) $con");
    $d2 = mysqli_fetch_array($ap);

    $p = $database->query("select count(id) as cnt from form5 WHERE status=4 $con");
    $d3 = mysqli_fetch_array($p);

    $r = $database->query("select count(id) as cnt from form5 WHERE status=3 $con");
    ...
    <table class="table table-bordered" style="table-layout:fixed;overflow:hidden">
        ...
        while($dis_data = mysqli_fetch_array($dis_sel)){
            ...
            $q1 = $database->query("select count(id) as cnt from form5 WHERE district='".$dis_data['uid']."' and status in (1,2,3,4) $c");
            $a1= mysqli_fetch_array($q1);

            $q2 = $database->query("select count(id) as cnt from form5 WHERE district='".$dis_data['uid']."' and status in (2,4) $c");
            $a2= mysqli_fetch_array($q2);

            $q3 = $database->query("select count(id) as cnt from form5 WHERE district='".$dis_data['uid']."' and status=3 $c");
            $a3= mysqli_fetch_array($q3);

            $q4 = $database->query("select count(id) as cnt from form5 WHERE district='".$dis_data['uid']."' and status=4 $c");
            $a4= mysqli_fetch_array($q4);

            $t = $database->query("select count(id) as cnt from form5 WHERE district='".$dis_data['uid']."' $c");
